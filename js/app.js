/* ==========================================================================
   Options Scanner — Main Application
   UI events, data orchestration, and rendering
   ========================================================================== */
...
(function () {
    ...
    const dom = {
        ...
        desiredHolding: $('desiredHolding'),
        maxContracts: $('maxContracts'),
        riskWillingness: $('riskWillingness'),
        upsideDesire: $('upsideDesire'),
        minDte: $('minDte'),
        maxDte: $('maxDte'),
        ...
        sumTarget: $('sumTarget'),
        sumMove: $('sumMove'),
        sumTimeline: $('sumTimeline'),
        sumConviction: $('sumConviction'),
        sumRisk: $('sumRisk'),
        ...
        wProb: $('wProb'),
        wForecast: $('wForecast'),
        wLiquidity: $('wLiquidity'),
        // Result stats
        resScanned: $('resScanned'),
        resBestScore: $('resBestScore'),
        resBestYield: $('resBestYield'),
        resAvgPop: $('resAvgPop'),
        ...
        try {
            ...
            try {
                ...
                if (liveData && liveData.chain.length > 0) {
                    ...
                    // Update price if we got fresh data
            ...
            const params = {
                ...
                riskWillingness: state.riskWillingness,
                upsideDesire: state.upsideDesire,
                direction: state.direction,
                strategy: state.strategy,
                timelineMonths: state.timeline,
                minDte: state.minDte,
                maxDte: state.maxDte
            };
    ...
    function renderTopPick(pick) {
        ...
        const stats = [
            { label: 'Premium (Mid)', value: `$${pick.mid.toFixed(2)}` },
            { label: 'Annualized Yield', value: `${pick.annualizedYield.toFixed(1)}%` },
            { label: 'Probability of Profit', value: `${pick.pop.toFixed(1)}%` },
            { label: 'Margin of Safety', value: `${pick.safetyPct.toFixed(1)}%` },
            { label: 'Break-Even', value: `$${pick.breakeven.toFixed(2)}` },
            { label: 'DTE', value: `${pick.dte} days` },
            { label: 'Delta', value: pick.delta.toFixed(4) },
            { label: 'Theta', value: `$${pick.theta.toFixed(4)}` },
            { label: 'IV', value: `${(pick.iv * 100).toFixed(1)}%` },
            { label: 'Max Profit / Contract', value: `$${pick.maxProfit.toFixed(2)}` },
            { label: 'Composite Score', value: pick.score.toFixed(1) }
        ];

        dom.topPickStats.innerHTML = stats.map(s =>
            `<div class="top-pick-stat">
                <span class="top-pick-stat-label">${s.label}</span>
                <span class="top-pick-stat-value">${s.value}</span>
            </div>`
        ).join('');

        // Generate rationale
        const safetyDesc = pick.safetyPct > 15 ? 'deep out-of-the-money' :
                          pick.safetyPct > 5 ? 'comfortably out-of-the-money' : 'near-the-money';
        const popDesc = pick.pop > 80 ? 'very high' : pick.pop > 65 ? 'high' : 'moderate';
        const yieldDesc = pick.annualizedYield > 30 ? 'exceptional' :
                         pick.annualizedYield > 15 ? 'strong' : 'reasonable';

        dom.topPickRationale.textContent =
            `This ${pick.type} at the $${pick.strike} strike is ${safetyDesc} with a ${popDesc} probability of profit (${pick.pop.toFixed(1)}%). ` +
            `It offers a ${yieldDesc} annualized yield of ${pick.annualizedYield.toFixed(1)}% with ${pick.dte} days until expiration. ` +
            `Your break-even price is $${pick.breakeven.toFixed(2)}, providing a ${pick.safetyPct.toFixed(1)}% buffer from the current price of $${state.currentPrice.toFixed(2)}.`;
    }
    ...
    function renderResults() {
        ...
        dom.resultsBody.innerHTML = limited.map((r, i) => {
            const rank = i + 1;
            return `<tr data-idx="${i}" onclick="window.__showDetail(${i})">
                <td>${rank}</td>
                <td><span class="cell-type ${r.type}">${r.type}</span></td>
                ...
                <td class="cell-yield">${r.annualizedYield.toFixed(1)}%</td>
                <td>${r.pop.toFixed(1)}%</td>
                <td>${r.safetyPct.toFixed(1)}%</td>
                <td>${r.delta.toFixed(3)}</td>
                <td>${r.theta.toFixed(4)}</td>
                <td>${(r.iv * 100).toFixed(1)}%</td>
                <td class="cell-score">${r.score.toFixed(1)}</td>
            </tr>`;
        }).join('');
    ...
    window.__showDetail = function (idx) {
        ...
        const items = [
            { label: 'Contract', value: r.contractSymbol },
            { label: 'Type', value: r.type.toUpperCase() },
            { label: 'Strike', value: `$${r.strike.toFixed(2)}` },
            { label: 'Expiration', value: r.expiry },
            { label: 'DTE', value: `${r.dte} days` },
            { label: 'Bid', value: `$${r.bid.toFixed(2)}` },
            { label: 'Ask', value: `$${r.ask.toFixed(2)}` },
            { label: 'Mid', value: `$${r.mid.toFixed(2)}` },
            { label: 'Last Price', value: `$${r.lastPrice.toFixed(2)}` },
            { label: 'Volume', value: r.volume.toLocaleString() },
            { label: 'Open Interest', value: r.openInterest.toLocaleString() },
            { label: 'IV', value: `${(r.iv * 100).toFixed(2)}%` },
            { label: 'Delta', value: r.delta.toFixed(4) },
            { label: 'Gamma', value: r.gamma.toFixed(4) },
            { label: 'Theta', value: `$${r.theta.toFixed(4)} / day` },
            { label: 'Vega', value: `$${r.vega.toFixed(4)}` },
            { label: 'Annualized Yield', value: `${r.annualizedYield.toFixed(2)}%` },
            { label: 'Probability of Profit', value: `${r.pop.toFixed(1)}%` },
            { label: 'Margin of Safety', value: `${r.safetyPct.toFixed(2)}%` },
            { label: 'Break-Even', value: `$${r.breakeven.toFixed(2)}` },
            { label: 'Max Profit / Contract', value: `$${r.maxProfit.toFixed(2)}` },
            { label: 'Max Loss / Contract', value: typeof r.maxLoss === 'string' ? r.maxLoss : `$${r.maxLoss.toFixed(2)}` },
            { label: 'Total Premium', value: `$${(r.mid * 100 * state.maxContracts).toFixed(2)}` },
            { label: 'In The Money', value: r.inTheMoney ? 'Yes' : 'No' },
            { label: 'Composite Score', value: r.score.toFixed(2) },
            { label: 'Premium Sub-Score', value: `${r.breakdown.premium}/100` },
            { label: 'Safety Sub-Score', value: `${r.breakdown.safety}/100` },
            { label: 'Probability Sub-Score', value: `${r.breakdown.probability}/100` },
            { label: 'Forecast Sub-Score', value: `${r.breakdown.forecast}/100` },
            { label: 'Liquidity Sub-Score', value: `${r.breakdown.liquidity}/100` },
            { label: 'Time Decay Sub-Score', value: `${r.breakdown.timeDecay}/100` }
        ];
