const OptionsEngine = (() => {
    ...
    function scoreOption(opt, params) {
        ...
        const premiumScore = Math.min(annualizedYield / 50, 1);
        let safetyPct;
        if (opt.type === 'put') {
        ...
        const score = (weights.premium * premiumScore + weights.safety * safetyScore + weights.probability * probScore + weights.forecast * forecastScore + weights.liquidity * liqScore + weights.timeDecay * timeDecayScore) * 100;
        const breakeven = opt.type === 'put' ? opt.strike - mid : opt.strike + mid;
        const maxProfit = mid * 100;
        let maxLoss;
        if (opt.type === 'put') {
            maxLoss = (opt.strike - mid) * 100;
        } else {
            maxLoss = Infinity;
        }
        return { ...opt, mid: Math.round(mid * 100) / 100, annualizedYield: Math.round(annualizedYield * 100) / 100, safetyPct: Math.round(safetyPct * 100) / 100, pop: Math.round(pop * 1000) / 10, breakeven: Math.round(breakeven * 100) / 100, maxProfit: Math.round(maxProfit * 100) / 100, maxLoss: maxLoss === Infinity ? 'Unlimited' : Math.round(maxLoss * 100) / 100, score: Math.round(score * 100) / 100, breakdown: { premium: Math.round(premiumScore * 100), safety: Math.round(safetyScore * 100), probability: Math.round(probScore * 100), forecast: Math.round(forecastScore * 100), liquidity: Math.round(liqScore * 100), timeDecay: Math.round(timeDecayScore * 100) }, weights };
    }
        ...
        for (const proxy of proxies) {
            try {
                ...
                if (allOptions.length > 0) {
                    return { source: 'Yahoo Finance (Live)', currentPrice, companyName: quote.shortName || quote.longName || ticker.toUpperCase(), chain: allOptions };
                }
            } catch {
                continue;
            }
    ...
    function parseYahooExpiration(expData, currentPrice) {
        ...
        const process = (contracts, type) => {
            ...
            contracts.forEach(c => {
                ...
                options.push({ type, strike: c.strike, bid: c.bid || 0, ask: c.ask || 0, lastPrice: c.lastPrice || 0, volume: c.volume || 0, openInterest: c.openInterest || 0, iv: c.impliedVolatility || 0, dte, expiry: expDate.toISOString().split('T')[0], inTheMoney: c.inTheMoney || false, contractSymbol: c.contractSymbol || '', delta: 0, gamma: 0, theta: 0, vega: 0 });
            });
        };
        ...
        expirations.forEach(expDate => {
            ...
            for (let K = minStrike; K <= maxStrike; K += step) {
                ...
                ['call', 'put'].forEach(type => {
                    ...
                    chain.push({ type, strike, bid, ask, lastPrice: Math.round(bs.price * 100) / 100, volume, openInterest, iv: Math.round(iv * 10000) / 10000, dte, expiry: expStr, inTheMoney: type === 'call' ? strike < currentPrice : strike > currentPrice, contractSymbol: `${ticker.toUpperCase()}${expStr.replace(/-/g, '')}${type === 'call' ? 'C' : 'P'}${(strike * 1000).toFixed(0).padStart(8, '0')}`, delta: Math.round(bs.delta * 10000) / 10000, gamma: Math.round(bs.gamma * 10000) / 10000, theta: Math.round(bs.theta * 10000) / 10000, vega: Math.round(bs.vega * 10000) / 10000 });
                });
            }
        });
        return chain;
    ...
    function enrichGreeks(chain, currentPrice, r = 0.045) {
        return chain.map(opt => {
            if (opt.delta !== 0 && opt.gamma !== 0) return opt;
            const T = opt.dte / 365;
            const mid = (opt.bid + opt.ask) / 2;
            let iv = opt.iv;
            if (!iv || iv === 0) {
            ...
            return { ...opt, iv: Math.round(iv * 10000) / 10000, delta: Math.round(bs.delta * 10000) / 10000, gamma: Math.round(bs.gamma * 10000) / 10000, theta: Math.round(bs.theta * 10000) / 10000, vega: Math.round(bs.vega * 10000) / 10000 };
        });
    }
    const DEMO_PRICES = {
    ...
    return { blackScholes, impliedVolatility, normalCDF, normalPDF, calculateWeights, scoreOption, scanOptions, fetchLiveData, generateSimulatedData, enrichGreeks, DEMO_PRICES };
})();